...
bool generate_changingpoint(int i, int Cnt1, Point start, Point end, Point changingpoint[], int d);
...
int main()
{
	...
	while(changingpoint[Cnt1-1].x != end.x and changingpoint[Cnt1-1].y != end.y)
	{
		Cnt1 += 1;
		bool flag = 0;		
		for(int i = 0; i < 8; i++)
		{
			// changing point
			bool available = generate_changingpoint(i, Cnt1, start, end, changingpoint, d);
...
bool generate_changingpoint(int i, int Cnt1, Point start, Point end, Point changingpoint[], int d)
{
	...
	if(i == 0)
	{
		changingpoint[Cnt1-1].x += 1 * x_sign;
		changingpoint[Cnt1-1].y += 0 * y_sign;
	}
	else if(i == 1)
	{
		changingpoint[Cnt1-1].x += 2 * x_sign;
		changingpoint[Cnt1-1].y += 0 * y_sign;		
	}
	else if(i == 2)
	{
		changingpoint[Cnt1-1].x += 0 * x_sign;
		changingpoint[Cnt1-1].y += 1 * y_sign;			
	}
	else if(i == 3)
	{
		changingpoint[Cnt1-1].x += 1 * x_sign;
	...
	{
		changingpoint[Cnt1-1].x += 0 * x_sign;
		changingpoint[Cnt1-1].y += 2 * y_sign;			
	}
	else if(i == 6)
	{
		changingpoint[Cnt1-1].x += 1 * x_sign;
	...
	{
		if(changingpoint[Cnt1-1].x >= end.x and changingpoint[Cnt1-1].y >= end.y)
		{
			return 0;
		}			
	}
